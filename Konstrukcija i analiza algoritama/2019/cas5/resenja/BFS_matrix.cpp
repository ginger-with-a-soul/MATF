...
public:
  ...
  /********* C++ deo *********/
  // Destruktori su nalik na konstruktore, samo sto sluze za oslobadjanje resursa (oslobadjanje memorije, zatvaranje datoteka ako su neke bile otvorene itd.)
  ...
  void BFS(int u)
  {
    ...
    while (!nodes.empty()) {
      ...
      for (int i = 0; i < V; i++) {
        ...
        if (!visited[i] && adjacency_matrix[pom][i])
          nodes.push(i);
      }
    }
